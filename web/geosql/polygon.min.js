...
function PolygonCreator(map) {
	...
	this.loadPolygon = function(data) {
		for (var index = 0; index < data.length; ++index) {
			thisOjb.pen.setCurrentDot(data[index]);
			data[index].remove();
			thisOjb.pen.draw(data[index].markerObj.getPosition());
		}
	}
...
function Pen(map) {
    ...
    this.draw = function (latLng) {
        if (null != this.polygon) {
            alert('Нажмите "Cтереть полигон", для того чтобы сделать новый');
        } else {
            if (this.currentDot != null && this.listOfDots.length > 1 && (this.currentDot.latLng.lat() == this.listOfDots[0].latLng.lat() && this.currentDot.latLng.lng() == this.listOfDots[0].latLng.lng())) {
